...
const UserController = {
  ...
  async create(req, res, next) {
    try {
      ...
      createAuditLog({ userId: req.user.id, action: 'CREATE', entityType: 'user', entityId: user.id, newValues: req.body, ipAddress: req.ip });
  ...
  update(req, res, next) {
    try {
      ...
      createAuditLog({ userId: req.user.id, action: 'UPDATE', entityType: 'user', entityId: req.params.id, oldValues: user, newValues: req.body, ipAddress: req.ip });
  ...
  deactivate(req, res, next) {
    try {
      ...
      createAuditLog({ userId: req.user.id, action: 'DEACTIVATE', entityType: 'user', entityId: req.params.id, ipAddress: req.ip });
